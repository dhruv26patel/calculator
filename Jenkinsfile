     ...
     stages {
          ...
          stage("Compile") {
            ...
            stage("Deploy to staging") {
                steps {
                    sh "pwd; cd calculator; ls -l; whoami; sudo su ec2-user; whoami; docker-compose up -d"
                }
            }
            stage("Acceptance test") {
                steps {
                    sleep 60
                    sh "pwd; ls -l; whoami; sudo su ec2-user; whoami; chmod +x acceptance_test.sh; ./acceptance_test.sh"
                }
            }
     }
    post {
